...
class Post extends Model
{
    ...
    public $belongsToMany = [
        'categories' => ['Lx\Book\Models\Category', 'table' => 'lx_book_posts_categories', 'order' => 'name']
    ];
    ...
    {
        return ($this->user_id == $user->id) || $user->hasAnyAccess(['lx.book.access_other_posts']);
    }
    public function beforeSave(){
    ...
    public function getUserNameOptions()
    {
        $users = new Users();
        $ad = array();
        $ad['no'] = 'no body borrow';
        $user = $users::all();
        foreach ($user as $us) {
        $usd =$us->attributes;
        $userName = $usd['name'];          
        $ad["$userName"] = $usd['name'];       
        }
        return $ad;
    }
